...
export class UserService {
  ...
  public async getUserProfile(id: string): Promise<{
    ...
    const queryResult = await this.userRepository
      ...
      .leftJoinAndSelect('user.partnerAccess', 'partnerAccess', 'partnerAccess.active = true')
      .leftJoinAndSelect('user.partnerAdmin', 'partnerAdmin', 'partnerAdmin.active = true')
    ...
    while (startIndex < users.length) {
      ...
      await Promise.all(
        batch.map(async (user) => {
          try {
            await this.frontChatService.deleteContact(user.email);
          } catch (error) {
            this.logger.warn(
              `deleteCypressTestUsers - unable to delete Front Chat contact for user ${user.id}: ${error?.message || 'unknown error'}`,
            );
          }
          try {
  ...
  public async deleteCypressTestUsers(clean = false): Promise<UserEntity[]> {
    ...
    try {
      // Clean remaining user accounts in firebase and mailchimp that do not have a user record in the db
