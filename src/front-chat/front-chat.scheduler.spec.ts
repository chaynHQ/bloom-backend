...
describe('FrontChatScheduler — unread notification flow', () => {
  ...
  beforeEach(async () => {
    chatUserService = {
      ...
      recoverStalePendingNotifications: jest.fn().mockResolvedValue(0),
    };
    serviceUserProfiles = {
  ...
  it('marks pending → sends Mailchimp event → marks sent for each eligible user', async () => {
    chatUserService.getUsersWithUnreadMessages.mockResolvedValue([
    ...
    expect(chatUserService.markUnreadNotificationPending).toHaveBeenCalledWith('cu-1');
    expect(chatUserService.markUnreadNotificationPending).toHaveBeenCalledWith('cu-2');
    ...
    expect(chatUserService.markUnreadNotificationSent).toHaveBeenCalledWith('cu-1');
    expect(chatUserService.markUnreadNotificationSent).toHaveBeenCalledWith('cu-2');
    expect(chatUserService.markUnreadNotificationFailed).not.toHaveBeenCalled();
  });
  ...
  it('marks pending before the Mailchimp call so a restart between the two cannot double-send', async () => {
    ...
    chatUserService.markUnreadNotificationPending.mockImplementation(async () => {
      order.push('pending');
    });
    serviceUserProfiles.sendMailchimpUserEventWithRecovery.mockImplementation(async () => {
      order.push('mailchimp');
    });
    chatUserService.markUnreadNotificationSent.mockImplementation(async () => {
      order.push('sent');
    });
  ...
  it('records the failure and does not mark sent when the Mailchimp event throws', async () => {
    ...
    expect(chatUserService.markUnreadNotificationPending).toHaveBeenCalledWith('cu-1');
    expect(chatUserService.markUnreadNotificationSent).not.toHaveBeenCalled();
    expect(chatUserService.markUnreadNotificationFailed).toHaveBeenCalledWith(
      'cu-1',
      'mailchimp 502',
    );
  });
  ...
  it('isolates failures per user — one failing send does not block the others in the batch', async () => {
    ...
    expect(chatUserService.markUnreadNotificationSent).toHaveBeenCalledWith('cu-1');
    expect(chatUserService.markUnreadNotificationFailed).toHaveBeenCalledWith('cu-2', 'boom');
    expect(chatUserService.markUnreadNotificationSent).toHaveBeenCalledWith('cu-3');
    expect(chatUserService.markUnreadNotificationSent).not.toHaveBeenCalledWith('cu-2');
  });
  ...
  it('skips the Mailchimp send entirely if the pending DB write fails, so we never send unrecorded', async () => {
    ...
    chatUserService.markUnreadNotificationPending.mockRejectedValueOnce(new Error('db down'));
