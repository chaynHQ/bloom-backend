...
- **Grounding now gets a `resource` row** (revised from decision 2): `RESOURCE_CATEGORIES.GROUNDING = 'grounding'` + `STORYBLOK_PAGE_COMPONENTS.RESOURCE_GROUNDING`; `resource_category_enum` includes `grounding`; webhook maps `resource_grounding → grounding`; webhook spec inverted.
- `reporting.events.ts` — "Resource events" now leads with the format-based families (`RESOURCE_VIDEO_*`, `RESOURCE_AUDIO_*`, `RESOURCE_WRITTEN_*`, `RESOURCE_ACTIVITY_*`, `RESOURCE_GROUNDING_VIEWED`; strings from 7a's working tree — `RESOURCE_VIDEO_VIDEO_STARTED` / `RESOURCE_AUDIO_AUDIO_STARTED`), keeps the `SHORT_VIDEO` / `SINGLE_VIDEO` / `CONVERSATION` lines flagged "(legacy)" for the GA transition window; "Resource errors" + `ANOMALY_WATCHED_EVENTS` gained the new names alongside the old. `RESOURCE_CATEGORY_LABELS` += `grounding`. `db-metrics` / `slack-blocks` spec fixtures moved `short_video → video`; snapshot regenerated. 7a + 7b ship together (no staging between the PRs); drop the legacy lines after ~2 cycles (step 8).
