...
## Current status — resume here (2026-09-09)

**Resource migration:** steps 1–5 **done**. Step 6 **in progress**:
...
**Step 7:** 7a (frontend) in progress. **7b (backend migration) code done 2026-09-10** — branch not yet merged; deploys after 7a reaches `main`. See the 7b section. **Amended 2026-09-10: 7b now also adds `resource_grounding` → a `resource` row (`category = 'grounding'`)** — reversing decision 2's "no row". Existing published grounding stories need a re-publish after 7b deploys to backfill their rows. 7c/7d not started.
...
| Grounding     | flat page + accordion                    | `/grounding`            | →   | `resource_grounding` | `/grounding?id=` (overlay) | `grounding`         |
...
| 2   | **Grounding:** one `/grounding` page, each exercise in an overlay addressed by `?id=<slug>`. No progress, no feedback, not in the library, not in the `Format` union. **Revised 2026-09-10 (step 7b): grounding stories DO get a backend `resource` row** with `category = 'grounding'` — dropping it was a mistake (it left `resource-user` / feedback unavailable and grounding invisible to reporting). The row exists; the frontend still opts out of progress/feedback UI.                                                                                                                                                                                                                                                                                                                  |     |
...
- `src/utils/constants.ts` — add `VIDEO/AUDIO/WRITTEN/ACTIVITY` to `RESOURCE_CATEGORIES` and `RESOURCE_{VIDEO,AUDIO,WRITTEN,ACTIVITY}` to `STORYBLOK_PAGE_COMPONENTS` (keep old members). ~~No `RESOURCE_GROUNDING` — grounding gets no row.~~ **Superseded 2026-09-10 (step 7b): `RESOURCE_GROUNDING` + `RESOURCE_CATEGORIES.GROUNDING` added — grounding gets a row after all.**
- `src/webhooks/webhooks.service.ts` `updateOrCreateStoryData` — recognise old + new resource components (~~not `resource_grounding`~~ — `resource_grounding` included as of step 7b); derive `category` from the component and **write it on update, not just create** (fixes the existing insert-only bug); keep the `undefined` fall-through for everything else.
...
- **Still to do, once backend PR #1257 is confirmed live on `main`**: `node scripts/storyblok/02-copy-exercise-content.mjs --publish --write --yes` (re-publishes every existing draft; throttled — 24 stories × up to 8 locales is a lot of webhook traffic; watch backend logs + Rollbar). Grounding produced no `resource` row at the time (pre-step-7b — grounding rows added in 7b; re-publish grounding after 7b deploys to backfill). **Then verify** every `resource_activity` story got a `resource` row (query the backend / DB by `storyblokUuid`) and re-publish any misses — `resource-user` and feedback endpoints return **404, not create-on-demand**, if the row is absent. **This verification is a hard gate on the step-6 frontend deploy.**
...
3. **Confirm every published `resource_activity` story has a `resource` row** (query by `storyblokUuid`); re-publish misses. Grounding rows were not expected at this point (added in step 7b).
...
- **Post-verify, only after production is confirmed green:**
  ...
  - `scripts/storyblok/06-strip-activity-slug-prefix.mjs --write --yes` — **ran 2026-09-09**: dropped the `activities-` prefix from all 8 `resource_activity` slugs (`activity/activities-thought-diaries` → `activity/thought-diaries`, etc.) + republished. The prefix was carried over verbatim from the old accordion ids (step 2); it's redundant under `activity/`. **No redirects added** (deliberate, per instruction) — old `/activity/activities-*` URLs and `?openacc=activities-*` deep links now 404. Verified on the published CDN: new slugs 200, old slugs 404. Internal links (live `full_slug`), uuid-based related refs (`related_content` / `related_grounding` / `related_session`), and uuid-keyed backend progress are all unaffected — no code consumer reads an activity slug literal.
    - **Frontend changes rode in PR #1958** (`home-welcome-pages`, "Home welcome pages transition" — the redesign-cutover appendix branch; the activity work was bundled in, commit `a5019696`): `public/sitemap.xml` updated to the 8 new `/activity/<slug>` URLs; `cypress/integration/user-content/activity.cy.tsx` — deleted the `?openacc=` deep-link test, repointed the render test to `/activity/thought-diaries`; this doc.
    - **⚠️ Production gap — resolve by merging #1958 to `main` promptly.** The Storyblok slugs are already renamed and published (live now), but `app/[locale]/activity/[slug]` has `dynamicParams = false`, so production only serves `/activity/<new-slug>` after a prod rebuild regenerates `generateStaticParams` from Storyblok's links API. Until #1958 reaches `main`: production still has the old static paths, and each `/activity/activities-*` page **hard-404s on its next ISR refresh** (`revalidate = 14400`, ≤4h) when Storyblok stops resolving the old slug — while the new `/activity/<slug>` paths don't exist yet. Staging (`develop`) is fine once #1958 is merged there. **Resume check:** after #1958 is on `main`, confirm `curl -sL https://bloom.chayn.co/activity/thought-diaries` → 200 and `/activity/activities-thought-diaries` → 404.
...
**Code done (2026-09-10), not yet merged — `bloom-backend`. Deploy only after 7a is on `main`.**

- Migration `BloomBackend1789025548195` (`src/migrations/1789025548195-bloom-backend.ts`, registered in `typeorm.config.ts`): the guarded `UPDATE`s + `CREATE TYPE resource_category_enum` + in-place `ALTER COLUMN … TYPE … USING` from the SQL below, verbatim. `down()` reverses the type to `character varying` then re-splits `video → single_video` / `audio → conversation` (lossy, accepted; `grounding` rows have no legacy equivalent — left as the free-text value `'grounding'`). Ran clean against the local dev DB (23 `video`, 5 `audio`, 0 `written`/`activity` rows → migrated with no loss; guard + reverse SQL both exercised manually) — **not re-run against a DB after the 2026-09-10 `grounding` amendment (single added enum member); run `yarn migration:run` locally before merge.**
- `ResourceEntity.category` → `@Column({ type: 'enum', enum: RESOURCE_CATEGORIES, nullable: false })`.
- `RESOURCE_CATEGORIES` trimmed to `VIDEO | AUDIO | WRITTEN | ACTIVITY | GROUNDING` (dropped `SHORT_VIDEO` / `SINGLE_VIDEO` / `CONVERSATION`) so the TS enum matches the PG type.
- `webhooks.service.ts` `RESOURCE_COMPONENT_CATEGORIES` — the legacy components (`resource_short_video` / `resource_single_video` → `video`, `resource_conversation` → `audio`) now derive the **merged** category, so a re-publish of a not-yet-moved story during the 7b→7c window writes a value the new enum accepts. `resource_grounding` → `grounding` (new — see the amendment note below); `RESOURCE_GROUNDING` added to `STORYBLOK_PAGE_COMPONENTS`. Component keys themselves stay until step 8's `07-cleanup`.
- **Amendment 2026-09-10 — grounding gets a `resource` row.** Reverses decision 2's "no backend `resource` row". `resource_grounding` now syncs like every other resource component: webhook derives `category = 'grounding'`, upserts a row keyed on `storyblokUuid`. Rationale: without a row, `resource-user` progress + feedback 404 for grounding, and grounding is absent from the Slack/DB reporting breakdown. Frontend still chooses not to render progress/feedback UI for grounding — that's a frontend decision, decoupled from the row existing. `grounding` added to `RESOURCE_CATEGORIES`, the migration guard set, the `resource_category_enum` members, and `RESOURCE_CATEGORY_LABELS` (`slack-blocks.builder.ts`). **After 7b deploys: re-publish the 16 existing `grounding/grounding-*` stories** (throttled Storyblok script or manual) to backfill their rows — no data migration can create them (the migration has no story data).
- Specs + `test/utils/mockData.ts` updated (legacy-component mock story now expects `VIDEO`); `yarn build` + full `yarn test` (367) + `yarn lint` green.
- `reporting.events.ts` — "Resource events" is now `Videos` / `Audio` / `Written` / `Activities` / `Grounding`. Each **item** sums the renamed GA event with its old name(s) (`EventItem.event` is now `string | string[]`; new helper `toEventNames`): `Videos → viewed` = `RESOURCE_VIDEO_VIEWED + RESOURCE_SHORT_VIDEO_VIEWED + RESOURCE_SINGLE_VIDEO_VIEWED`, etc.; `Audio` folds in `RESOURCE_CONVERSATION_*`. So the line (and its summed baseline) stays continuous across the 7a cutover — no split lines, no double-counting, no manual addition. "Resource errors" merges the same way. `renderedEventNames()` / the GA breakdown `inListFilter` flatten the arrays, so the persisted `reporting_run.ga4Events` row still stores every name granularly (summing is render-only). `RESOURCE_CATEGORY_LABELS` += `grounding`; `db-metrics` / `slack-blocks` spec fixtures moved `short_video → video`. `ANOMALY_WATCHED_EVENTS` keeps old + new as separate per-event watches (per-event baseline variance matters for anomaly scoring; not summed). Step 8 = delete the legacy names from each merged `event` array + drop the now-redundant legacy anomaly watches. 7a + 7b ship together.
  - **Amendment 2026-09-10 — double-word transition era.** The 7a frontend event fix also stopped `<Video>` / `<ResourceAudioPlayer>` hard-coding the `_VIDEO_` / `_AUDIO_` infix — the media word now lives in each caller's `eventPrefix` (`SESSION_VIDEO`, `COURSE_INTRO_VIDEO`, `RESOURCE_VIDEO`, …), so `Session` / `Course intro` events keep their exact GA strings and only the resource media events change. In the gap between the redesign landing on `develop` (2026-09-09) and that fix, the new `audio` page (and, if its preview deploy fired events, the 7a `video` page) emitted the doubled `RESOURCE_AUDIO_AUDIO_*` / `RESOURCE_VIDEO_VIDEO_*`. **Deliberately not folded into `reporting.events.ts`** — ~1–2 days of mostly-staging traffic, not worth carrying. Separately, `Resource → session navigation` gained a `from audio` line (`RESOURCE_AUDIO_VISIT_SESSION` + legacy `RESOURCE_CONVERSATION_VISIT_SESSION`) — the new audio page fires that permanent event and nothing was reporting it.
...
- `resource-user` progress + feedback now work for grounding too (they need the row) — a user-facing gain, though the frontend still hides that UI for grounding. `down()` is lossy (`video → single_video`, `audio → conversation`; `grounding` stays `'grounding'`) — acceptable.
- Entity → `@Column({ type: 'enum', enum: RESOURCE_CATEGORIES })`; trim old `RESOURCE_CATEGORIES` members (and add `GROUNDING`) so the TS enum matches the PG type.
...
- **bloom-frontend:** delete old i18n keys, the old→new event comment, `EXERCISE_CATEGORIES` if unused (still used — `GroundingExerciseDialog.tsx` sets `resource_category: EXERCISE_CATEGORIES.GROUNDING`; keep unless that event is renamed to `RESOURCE_GROUNDING`), `STORYBLOK_TAGS.SOMATICS` (still used in `getLibraryStories.ts`), `bloom_shorts.png`.
...
- **`next.config.js` redirects** — remove the two now-dead `/activities?openacc=<slug>` → `/activity/:slug` rules (bare + `/${LOCALE_PATTERN}/`): the activity slug strip (step 6 post-verify) made their targets 404, and no 301s were added to bridge them. **Keep** `/activities` → `/library?format=activity` (permanent). The video/audio folder redirects added in 7a stay.
- **Storyblok** — `scripts/storyblok/07-cleanup.ts` (07- because `06-` is taken by the activity slug-strip script): `DELETE /components/:id` for `resource_short_video` / `resource_single_video` / `resource_conversation`; `DELETE /stories/:id` for the empty `shorts/` `conversations/` `videos/` folders; `PUT /components/:id` to drop the `related_exercises` field from any block still declaring it.
...
- **Deliberately retained (not cleanup targets):** the `?openacc=` query alias on `/grounding` (`StoryblokGrounding` reads `?openacc` as a synonym for `?id`) — a cheap back-compat alias for old grounding deep links, keep indefinitely. `LEGACY_THEME_KEYS` in `libraryData.ts` (`recognising-harm`/`why-harm-happens` → `-abuse`) — old `?theme=` deep links and any not-yet-re-saved Storyblok story still carry the old slugs; only remove once every published story is confirmed on the new slugs and the old deep links are cold.
...
- **`eventPrefix` double-word** — the four resource families stay separate (`RESOURCE_VIDEO` / `RESOURCE_AUDIO` / `RESOURCE_WRITTEN` / `RESOURCE_ACTIVITY`). Target media sub-events read `RESOURCE_VIDEO_STARTED` / `RESOURCE_AUDIO_STARTED`. Step 7's frontend fix: `<Video>` / `<ResourceAudioPlayer>` no longer append `_VIDEO_` / `_AUDIO_` themselves — the caller's `eventPrefix` carries the media word (so `SESSION_VIDEO`, `COURSE_INTRO_VIDEO`, `STORYBLOK_VIDEO`, `RESOURCE_VIDEO`; `STORYBLOK_AUDIO`, `RESOURCE_AUDIO`). Non-resource lines (`Session video`, `Course intro video`) keep their exact GA strings. The pre-fix double-word names (`RESOURCE_{VIDEO,AUDIO}_{VIDEO,AUDIO}_*`, plus the old `RESOURCE_SINGLE_VIDEO_VIDEO_*` / `RESOURCE_CONVERSATION_AUDIO_*`) are all folded into `reporting.events.ts`.
...
- **`resource-user` / feedback are not create-on-demand** — they 404 if no `resource` row exists for the `storyblokUuid`. Every new `resource_activity` / `resource_written` / `resource_grounding` story must be confirmed to have produced a row (webhook) before it's linked anywhere users can complete it. Grounding rows only exist from step 7b onward — re-publish the existing grounding stories once 7b is deployed.
...
- Publishing a resource upserts the `resource` row with the right `category`; grounding publishes upsert a row with `category = 'grounding'` (as of step 7b).
