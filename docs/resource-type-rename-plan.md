...
- **`eventPrefix` double-word** — the four resource families stay separate (`RESOURCE_VIDEO` / `RESOURCE_AUDIO` / `RESOURCE_WRITTEN` / `RESOURCE_ACTIVITY`), so the media sub-events read `RESOURCE_VIDEO_STARTED` / `RESOURCE_AUDIO_STARTED` (family prefix + `<Video>`/`<Audio>` suffix). That's the target — the only fix in step 7 is the `/shorts` page passing `"RESOURCE_SHORT"` instead of the family prefix. Match `reporting.events.ts`.
